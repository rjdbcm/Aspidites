...
def expand_state_definition(source, loc, tokens):
    ...
    statedef.extend(
        "{0}._next_state_class = {1}".format(s, fromTo[s])
        for s in states
        if s in fromTo
    )
...
def expand_named_state_definition(source, loc, tokens):
    ...
    for s in states:
        ...
        statedef.extend(
            "%s.%s = classmethod(lambda cls: %s())" % (s, tn_, to_) for tn_, to_ in trns
        )
        ...
        [
            "{baseStateClass}.transitions = classmethod(lambda cls: [{transition_class_list}])".format(
                ...
                transition_class_list=", ".join(
                    "cls.{0}".format(tn) for tn in transitions
                ),
            ),
        ...
        for dirpath in sys.path:
            ...
            if isinstance(finder, (type(None), importlib.machinery.FileFinder)):
                checkpath = os.path.join(
                    dirpath, "{0}.{1}".format(fullname, self.suffix)
                )
                yield checkpath
