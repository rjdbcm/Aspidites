...
class MatchFirst(ParseExpression):
    ...
    def parseImpl(self, instring, loc, doActions=True):
        maxExcLoc = -1
        maxException = None
        instringlen: int = len(instring)
        for e in self.exprs:
            try:
                ret = e._parse(instring, loc, doActions)
                return ret
            except (ParseException, ValueError) as err:
                if type(err) == ParseException:
                    if err.loc > maxExcLoc:
                        maxException = err
                        maxExcLoc = err.loc
                if type(err) == ValueError:
                    if instringlen > maxExcLoc:
                        maxException = ParseException(instring, instringlen, e.errmsg, self)
                        maxExcLoc = instringlen
